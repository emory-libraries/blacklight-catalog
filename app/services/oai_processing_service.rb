...
class OaiProcessingService
  ...
  def self.pull_active_ids(document, suppressed_ids, logger)
    ids = document.xpath(active_ids_xpath, oai_url.merge(marc_url)).map(&:content) - [*suppressed_ids]
    logger.info "Active IDs: #{ids}"
    ids
  end

  def self.active_ids_xpath
    '/oai:OAI-PMH/oai:ListRecords/oai:record/oai:metadata/marc:record/marc:controlfield[@tag="001"]'
  end

  def self.solr
    RSolr.connect(url: ENV['SOLR_URL'], update_format: :xml, retry_503: 5, retry_after_limit: 5)
  end
