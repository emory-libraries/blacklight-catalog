...
CatalogController.class_eval do
  def index
    ...
    @documents_availability = AlmaAvailabilityService.new(@response.documents.map(&:id)).availability_of_documents unless request.fullpath == '/' || request.fullpath.include?('profile=') && !request.fullpath.include?('q=')
