  ...
  def extract_publication_main_display
    lambda do |rec, acc|
      ...
      unless extra_fields.flatten.empty?
        ...
        process_field = selected_field["245fg"]&.first # get value of 245 field, because this may contain duplicates and will need processing
        ...
        selected_field = selected_field.class == String ? selected_field : selected_field.max_by(&:length)
        acc << selected_field
      end
    end
  end
